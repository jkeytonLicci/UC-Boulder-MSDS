...
This analysis uses COVID19 data from John Hopkins' Github site. After accessing the data, we tidy the data following
guidance from the lectures. In particular, we aggregate the number of cases and death by location from the start of COVID19 through
March 2023. 

The focus of this analysis is on the deaths per 1,000 people in the population of each state or province. The question we try to answer is: *Does the population size
influence the deaths per 1,000 people?* The reason I find this question interesting is that the "per 1,000" is often use to normalize a feature in the population in
order to better understand the feature. It is the same reason one considers "per capita" when evaluating different regional metrics. In this case, viral diseases might flourish in denser populations, which would then disproportionately affect the number of deaths. Our basic analysis tries to examine this idea.
